...
static uint8_t dimmer_level_ch1 = 50;           // Fixed dimmer level for channel 1 (50%)
...
static void IRAM_ATTR dimmer_zero_cross_isr(void* arg) {
    ...
    // For channel 1
    if (dimmer_enabled) {  // Check if dimmer is enabled
        if (dimmer_level_ch1 > 0 && dimmer_level_ch1 < 100) {
            // Convert percentage to delay (more delay = less power)
            ...
            // If full power, keep triac on
            if (dimmer_level_ch1 >= 99) {
                gpio_set_level(DIMMER_CH1_PIN, 1);
                debug_triac_triggers++;  // Count TRIAC trigger
            } else if (dimmer_level_ch1 <= 1) {
                gpio_set_level(DIMMER_CH1_PIN, 0);
            }
        }
        else if (dimmer_level_ch1 >= 100) {
            // Full power
            gpio_set_level(DIMMER_CH1_PIN, 1);
            debug_triac_triggers++;  // Count TRIAC trigger
        }
        ...
        // Dimmer disabled - turn on full power
        gpio_set_level(DIMMER_CH1_PIN, 1);
        debug_triac_triggers++;  // Count TRIAC trigger
    }
    
    // Channel 2 is not used in this application
    // Set channel 2 to always off
    gpio_set_level(DIMMER_CH2_PIN, 0);
}

// Timer interrupt handler for triac firing
static void IRAM_ATTR dimmer_timer_isr(void* arg) {
    ...
    if (dimmer_enabled && dimmer_level_ch1 > 0 && dimmer_level_ch1 < 100) {
        ...
        // Reset triac after a short pulse (300µs is better than 100µs for reliability)
        ets_delay_us(300);  // Increased pulse width for better TRIAC triggering
        gpio_set_level(DIMMER_CH1_PIN, 0);
    }
    
    // Similar logic for channel 2 if needed
}
...
static void dimmer_control_task(void* pvParameters) {
    ...
    // Para pruebas: configurar el dimmer al 100% para verificar funcionamiento
    dimmer_level_ch1 = 80;  // 100% de potencia para pruebas iniciales
...
// Esta función se usa en dimmer_init para mostrar el nivel inicial
#ifdef UNUSED_FUNCTION
static void set_dimmer_level(uint8_t channel, uint8_t level) {
...
void dimmer_init(void) {
    ...
    // Establecer nivel de potencia inicial (100% para pruebas)
    dimmer_level_ch1 = 80;
    
    ESP_LOGI(TAG, "AC Dimmer initialized at 100%% brightness for testing");
    
    // Verificar que el GPIO está bien configurado
...
char* DHT_reader_task(void)
{
    ...
    cJSON_AddNumberToObject(dimmer, "channel1", dimmer_level_ch1);
    cJSON_AddNumberToObject(dimmer, "channel2", dimmer_level_ch2);
    cJSON_AddBoolToObject(dimmer, "enabled", dimmer_enabled);
...
void app_main()
{
    ...
    // El nivel del dimmer ya está configurado al 50% como variable global
...
// CODIGO QUE SIRVE PERO CON PARAPDEOS//
